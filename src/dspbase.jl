...
function filt!(out::AbstractArray, b::Union{AbstractVector, Number}, a::Union{AbstractVector, Number},
    ...
    if size(x) != size(out)
        s_fun = @static VERSION < v"1.8" ? string : LazyString
        throw(ArgumentError(s_fun("output size ", size(out), " must match input size ", size(x))))
    end
...
dsp_reverse(v::AbstractVector, ::Tuple{Base.OneTo{Int}}) = reverse(v; dims=1)
...
```
"""
xcorr(u::AbstractVector, v::AbstractVector; kwargs...) = _xcorr(u, v; kwargs...)

@inline function _xcorr(
...
@inline function _autocorr_fft(u::AbstractVector{T}) where T<:FFTTypes
...
xcorr(u::AbstractVector; kwargs...) = _xcorr(u, u; kwargs...)
